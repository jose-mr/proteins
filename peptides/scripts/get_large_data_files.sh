...
wget -O uniprot_peptides.dat.gz "https://rest.uniprot.org/uniprotkb/stream?format=txt&compressed=true&query=%28length%3A%5B1+TO+100%5D%29"

echo "create files with all peptide accession codes in uniprot"
zgrep "^AC" uniprot_peptides.dat | awk -F "   " '{print $2}' | tr -d ";" | tr " " "\n" > peptides_acs.txt

# filter go annotations
